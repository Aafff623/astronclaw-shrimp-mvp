...
| 变更文件模式 | 归属变更源 |
|-------------|-----------|
| `skills/*.yaml` 或 `skills/*.json` | Skill 配置变更（链路 A） |
| `.env.example` | 环境变量结构变更（链路 A 衍生） |
| `deploy.md` | 部署流程变更（链路 B） |
| `docs/技术报告.md` | 技术架构变更（链路 D） |
| `screenshots/*` 或 `docs/效果截图/*` | 演示资料更新（链路 C） |
| `docs/演示视频.md` | 视频链接更新（链路 C） |
| `README.md` | 结构 / 首屏调整 |
| `GUIDE.md` / `CLAUDE.md` / `steering.md` | 三层规范变更（链路 E） |
| `.claude/skills/*/SKILL.md` 或 `.kiro/skills/*/SKILL.md` | Skill 双套变更（链路 F） |
| `memory/*.md` | Memory 新增 / 更新（链路 G） |

只检查对应链路的下游文件。

**模式 B：主动巡检**（git status 干净）

遍历所有 7 条链路，逐一检查一致性。重点检查：
- README 部署链接 / 部署二维码 是否与最新部署一致
- README 视频链接 是否与 `docs/演示视频.md` 一致
- README 仓库结构树 是否与实际目录一致
- `.env.example` 列表是否覆盖了 Skill 中引用的所有变量
- `CLAUDE.md` 与 `steering.md` 主体是否对称
- `.claude/skills/` 与 `.kiro/skills/` 是否字节级一致
- `memory/MEMORY.md` 索引是否与实际 memory 文件一致
...
#### 链路 E：三层规范对齐（多 Agent 协作）

```
变更源: GUIDE.md / CLAUDE.md / steering.md
↓
1. CLAUDE.md ↔ steering.md     → 主体保持对称（"核心目标 / 阶段 / 决策 / 优先级"完全一致）
                                  差异仅在最后的"行为约定"节（工具名）
2. GUIDE.md                    → 公共规范，两入口都要正确引用
3. memory/                     → 如涉及行为偏好，更新对应 feedback / user memory
```

检查项：
- [ ] CLAUDE.md 与 steering.md 的"核心目标 / 阶段 / 决策 / 优先级"是否完全一致？
- [ ] 差异是否仅在最后的"行为约定"节？
- [ ] GUIDE.md 的引用链接（`.claude/skills/...`）是否仍然有效？
- [ ] 三个文件是否都引用了对应链接（互相可达）？

---

#### 链路 F：Skill 双套同步（Claude Code ↔ Kiro）

```
变更源: .claude/skills/<name>/SKILL.md 或 .kiro/skills/<name>/SKILL.md
↓
1. 另一个位置的同名 skill      → 字节级一致
   - .claude/skills/ 改了 → .kiro/skills/ 跟着改
   - .kiro/skills/ 改了 → .claude/skills/ 跟着改
2. memory/                     → 新经验沉淀（feedback / project 类型）
```

操作建议：用 `Copy-Item -Force` 直接覆盖保证一致，比手动同步可靠。

检查项：
- [ ] `.claude/skills/<name>/SKILL.md` 与 `.kiro/skills/<name>/SKILL.md` 内容是否一致？
- [ ] SKILL.md 的 `description` 是否清晰（触发条件足够明确）？
- [ ] 新经验是否需要沉淀到 `memory/`？
...
```
变更源: 重要决策产生（commit message 含"决策 / 判断 / 推荐"，或对话中明确表态）
↓
1. memory/<type>_<topic>.md    → 新增或更新对应 memory
2. memory/MEMORY.md            → 索引同步（每条一行）
3. docs/design-thinking.md     → 重大设计判断追加到设计思想总结
```

判断"是否值得 memory"：
- ✅ 该值得：设计判断、工具偏好、风格偏好、反向决策（推翻原方案）、外部参考
- ❌ 不值得：临时任务状态、git 已记录的代码变更、CLAUDE.md 已覆盖的规范

检查项：
- [ ] 决策是否值得 memory（不是临时状态）？
- [ ] frontmatter `type` 是否合理（user / feedback / project / reference）？
- [ ] `MEMORY.md` 索引是否同步更新？
- [ ] 是否用 `[[name]]` 链接到相关 memory？
